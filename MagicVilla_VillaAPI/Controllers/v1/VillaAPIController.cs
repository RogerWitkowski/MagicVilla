    ...
    public class VillaAPIController : ControllerBase
    {
            ...
            [FromQuery] string? search, int pageSize = 0, int pageNumber = 1)
        {
            try
            {
                ...
                if (occupancy > 0)
                {
                    villaList = await _dbVillaRepository.GetAllAsync(u => u.Occupancy == occupancy, pageSize: pageSize, pageNumber: pageNumber);
                }
                else
                {
                    villaList = await _dbVillaRepository.GetAllAsync(pageSize: pageSize, pageNumber: pageNumber);
                }
